...
def checkapk():
    ...
    for filename in os.listdir('static'):
        if os.path.isfile(os.path.join('static', filename)):
            print(f"this is the name of my file {filename}")
    file_to_check = os.path.join("static", filename)
    ...
    if os.path.exists(model_path):
        ...
        if result == 1:
            ...
            return render_template('index.html',message="Identified as Malware !")
        else:
            print("Analysed file '{}', identified as not malware.".format(file_to_check))
            return render_template('index.html',message="Identified as  Not Malware !")
    ...
    raise Exception("An error occurred while analysing the APK")
    ...
    if len(sys.argv) > 1:
        ...
        raise Exception("Please provide an APK to analyse")
        return "Please provide an APK to analyse"
    ...
    if os.path.exists(model_path):
        ...
        if len(sys.argv) > 2:
            ...
            if dst_file.endswith(".json"):
                ...
                raise Exception("A destination file was provided but it was not a Json file.")
                return "A destination file was provided but it was not a Json file"
